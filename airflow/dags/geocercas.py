"""DAG ``geocercas`` - ingestion of the geofence registry.
...
First phase: only the raw ingestion task. The staging stage (table
config, treatments, validations and the ``staging_geocercas`` Asset)
will be added in a later phase.

The DAG is thin on purpose: all PySpark logic lives in
``pipelines/geocercas/geocercas.py``.
...
from pipelines.geocercas.geocercas import RAW_DIR, extract_to_raw
...
@dag(
    dag_id="geocercas",
    description="Geofence registry ingestion (GeoJSON -> raw)",
    schedule="*/10 * * * *",
    start_date=pendulum.datetime(2024, 1, 1, tz="UTC"),
    catchup=False,
    max_active_runs=1,
    default_args=DEFAULT_ARGS,
    tags=["bigcore", "geocercas", "raw"],
    doc_md=__doc__,
)
def geocercas():
