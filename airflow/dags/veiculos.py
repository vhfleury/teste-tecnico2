"""DAG ``veiculos`` - ingestion and quality of the vehicle registry.
...
``pipelines/veiculos/veiculos.py`` and the staging contract
(columns, treatments, validations, key) in ``staging_veiculos.json``.
...
from pipelines.veiculos.veiculos import (
    RAW_DIR,
    STAGING_DIR,
    extract_to_raw,
...
def veiculos():
    ...
    def transform_to_staging_task(**context) -> dict:
        ...
        metrics = run_spark("veiculos_transform", transform_to_staging, ingest_date)
        log.info(
