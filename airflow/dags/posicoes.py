"""DAG ``posicoes`` - ingestion of the vehicle tracking positions.
...
``pipelines/posicoes/posicoes.py``. For now only the raw ingestion task
exists; the staging transform (cleaning/validation driven by the table
config) will be added next.
...
from pipelines.posicoes.posicoes import RAW_DIR, extract_to_raw
...
@dag(
    dag_id="posicoes",
    description="Vehicle tracking positions ingestion (Parquet -> raw)",
    schedule="*/10 * * * *",
    start_date=pendulum.datetime(2024, 1, 1, tz="UTC"),
    catchup=False,
    max_active_runs=1,
    default_args=DEFAULT_ARGS,
    tags=["bigcore", "posicoes", "raw"],
    doc_md=__doc__,
)
def posicoes():
