"""DAG ``viagens`` - ingestion and quality of the trips fact source.
...
Flow across the lakehouse layers, partitioned by ingestion date::
    ...
    data/viagens/viagens.csv
      |-(extract_to_raw)->  lakehouse/raw/viagens/ingest_date=YYYY-MM-DD
          |-(transform_to_staging)->  lakehouse/staging/viagens/ingest_date=YYYY-MM-DD

The DAG is thin on purpose: all PySpark logic lives in
``pipelines/viagens/viagens.py`` and the staging contract
(columns, treatments, validations, key) in ``staging_viagens.json``.
...
The final task publishes the ``staging_viagens`` Asset, the data-aware
...
from pipelines.viagens.viagens import (
    RAW_DIR,
    STAGING_DIR,
    extract_to_raw,
...
def viagens():
    ...
    def transform_to_staging_task(**context) -> dict:
        ...
        metrics = run_spark("viagens_transform", transform_to_staging, ingest_date)
        log.info(
