"""DAG ``motoristas`` - ingestion and quality of the driver registry.
...
Flow across the lakehouse layers, partitioned by ingestion date::
    ...
    data/motoristas/motoristas.json
      |-(extract_to_raw)->  lakehouse/raw/motoristas/ingest_date=YYYY-MM-DD
          |-(transform_to_staging)->  lakehouse/staging/motoristas/ingest_date=YYYY-MM-DD

The DAG is thin on purpose: all PySpark logic lives in
``pipelines/motoristas/motoristas.py`` and the staging contract
(columns, treatments, validations, key) in ``staging_motoristas.json``.
...
The final task publishes the ``staging_motoristas`` Asset, the data-aware
...
from pipelines.motoristas.motoristas import (
    RAW_DIR,
    STAGING_DIR,
    extract_to_raw,
...
@dag(
    ...
    tags=["bigcore", "motoristas", "staging"],
    doc_md=__doc__,
)
def motoristas():
    ...
    def extract_to_raw_task(**context) -> dict:
        ...
        metrics = run_spark("motoristas_extract", extract_to_raw, ingest_date)
        log.info("Raw layer written: %s", metrics)
        return metrics

    @task(
        task_id="transform_to_staging",
        outlets=[STAGING_MOTORISTAS],
    ...
    def transform_to_staging_task(**context) -> dict:
        ...
        metrics = run_spark("motoristas_transform", transform_to_staging, ingest_date)
        log.info(
