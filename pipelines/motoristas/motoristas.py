"""motoristas pipeline: the driver registry.
...
def clean_and_validate(raw: DataFrame, config: dict) -> DataFrame:
    ...
    keeps its primary key), and finally the derived columns - so a
    derived value (e.g. the digits-only CPF) is computed from the
