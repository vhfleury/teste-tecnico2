"""Compatibility wrapper for the generic position staging pipeline."""
...
def extract_to_raw(spark: SparkSession, ingest_date: str) -> dict:
    """Read the tracking positions source and write the raw partition."""
    return _extract_to_raw(spark, SOURCE, ingest_date)


def transform_to_staging(spark: SparkSession, ingest_date: str) -> dict:
    """Read raw tracking positions, apply the table config and write staging."""
